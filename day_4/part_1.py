...
def part_1(called: list, boards: list):
...
def part_2(called:list, boards:list):
    while len(boards) > 1:
        winning_num, winning_board = find_winning_board_and_number(called, boards)
    ...
    print(f'Last winning number: {winning_num}')
    print(f'Last winning board: {winning_board}')
...
if __name__ == '__main__':
    ...
    boards_raw = '\n'.join(lines[2:]).split('\n\n')
    boards = make_boards(boards_raw)
    
    part_1(called, boards)
    part_2(called, boards)
